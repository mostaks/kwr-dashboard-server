...
export const createDashboardService = async (body: {
  ...
  try {
    const { name, tagCategories, keywords } = body;
    // Start a new batch
    const batch = db.batch();
    // Create dashboard document
    const dashboardRef = db.collection('dashboards').doc();
    batch.set(dashboardRef, { name });
    // Create tag categories and tags
    const tagCategoryRefs = [];
    for (const category of tagCategories) {
      const categoryRef = db.collection('tagCategories').doc();
    ...
    for (const keyword of keywords) {
      const keywordRef = db.collection('keywords').doc();
      ...
      batch.set(keywordRef, {
        ...
        data: keyword.data,
        tags: keyword.tags.map((tag: any) => db.collection('tags').doc(tag.id)),
